    ...
    return [
      {
        ...
        items: await this.basicFetch(
          `/discover/tv?${this.OriginaisNetflix}&${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'trending',
        ...
        items: await this.basicFetch(
          `/trending/all/week?${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'toprated',
        title: 'Em Alta',
        items: await this.basicFetch(
          `/movie/top_rated?${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'action',
        title: 'Ação',
        items: await this.basicFetch(
          `/discover/movie?${this.generoAcao}&${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'comedy',
        title: 'Comédia',
        items: await this.basicFetch(
          `/discover/movie?${this.generoComedia}&${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'horror',
        title: 'Terror',
        items: await this.basicFetch(
          `/discover/movie?${this.generoTerror}&${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'romance',
        title: 'Romance',
        items: await this.basicFetch(
          `/discover/movie?${this.generoRomance}&${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
      {
        slug: 'documentary',
        title: 'Documentários',
        items: await this.basicFetch(
          `/discover/movie?${this.Documentarios}&${this.Idioma}&api_key=${this.apiKey}`
        ),
      },
    ];
  }
      ...
      switch (type) {
        case 'movie':
          info = await this.basicFetch(
            `/movie/${movieId}?${this.Idioma}&api_key=${this.apiKey}`
          );
          break;
        case 'tv':
          info = await this.basicFetch(
            `/tv/${movieId}?${this.Idioma}&api_key=${this.apiKey}`
          );
          break;
        default:
          info = null;
